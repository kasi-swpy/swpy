...
def download(uid, start_date = "", end_date = "", overwrite = False):
    ...
    download_kasi_id = "download_kasi_mm_" + uid
    ...
    if(start_date_format == None or end_date_format == None):
        print("KASI_MM download() : Wrong date format. (start_date len : %s, end_date len: %s)"%(len(start_date), len(end_date)))
        return False
    ...
    try:
        ...
        print(e)
        print("KASI_MM download() : Wrong start date.")
        return False
    try:
        end_d = datetime.strptime(end_date, end_date_format)
    except Exception as e:
        print(e)
        print("KASI_MM download() : Wrong end date.")
        return False
    ...
    # multi day
        ...
        if(start_d > end_d):
            print("KASI_MM download() : The end date comes before the start date.")
            return False
        delta = end_d - start_d
    ...
    # Download from Start date to end date (period : day).
...
# download([], data_info_dict, "20200129", "2020")
